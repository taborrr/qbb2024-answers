...
for line in file:
    ...
    if fields[2]== "gene":
        ...
        text = fields[8]
        get_genename = text.split(' ')
        gname = get_genename[5]
        extracted_string = gname.strip('";')
        
        ## method below did not use fields approach
        ...
        print(str(fields[0]) + '\t' + str(fields[3]) + '\t' + str(fields[4]) + '\t' + extracted_string)# extracted_value) 
    else:
        continue
